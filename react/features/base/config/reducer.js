...
const INITIAL_RN_STATE = {
    ...
    // FIXME The support for audio levels in lib-hellotter-app polls the statistics
...
function _setConfig(state, { config }) {
    // The mobile app bundles hellotter-app and lib-hellotter-app at build time and
 ...
 * config {@code Object} which is in the latest format supported by hellotter-app.
 * Such a translation from an old config format to a new/the latest config
 * format is necessary because the mobile app bundles hellotter-app and
 * lib-hellotter-app at build time and does not download them at runtime from the
 * deployment on which it will join a conference.
 *
 * @param {Object} oldValue - The config {@code Object} which may or may not be
 * in the latest form supported by hellotter-app and from which a new config
 * {@code Object} is to be constructed if necessary.
 * @returns {Object} A config {@code Object} which is in the latest format
 * supported by hellotter-app.
 */
