 ...
 * (which signalizes that we finally left the app) and disposes lib-hellotter-app.
 * Also captures SET_CONFIG action and disposes previous instance (if any) of
 * lib-hellotter-app, and initializes a new one with new config.
 *
...
/**
 * Notifies the feature base/lib-hellotter-app that the action SET_CONFIG is being
...
function _setConfig({ dispatch, getState }, next, action) {
    const { initialized } = getState()['features/base/lib-hellotter-app'];

    // XXX Since the config is changing, the library lib-hellotter-app must be
    // initialized again with the new config. Consequently, it may need to be
    // disposed of first.
    // TODO Currently, disposeLib actually does not dispose of lib-hellotter-app
    // because lib-hellotter-app does not implement such functionality.
    if (initialized) {
        dispatch(disposeLib());
    }
