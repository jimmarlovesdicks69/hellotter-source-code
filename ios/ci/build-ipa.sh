...
# LIB_JITSI_MEET_PKG (optional) - the npm package for lib-hellotter-app which will
...
# Link this lib-hellotter-app checkout in hellotter-app through the package.json
...
then
    ...
    grep lib-hellotter-app package.json
else
...
mkdir -p /tmp/hellotter-app/

xcodebuild archive -quiet -workspace ios/hellotter-app.xcworkspace -scheme hellotter-app -configuration Release -archivePath /tmp/hellotter-app/hellotter-app.xcarchive

sed -e "s/YOUR_TEAM_ID/${IOS_TEAM_ID}/g" ios/ci/build-ipa.plist.template > ios/ci/build-ipa.plist

IPA_EXPORT_DIR=/tmp/hellotter-app/hellotter-app-ipa

xcodebuild -quiet -exportArchive -archivePath /tmp/hellotter-app/hellotter-app.xcarchive -exportPath $IPA_EXPORT_DIR  -exportOptionsPlist ios/ci/build-ipa.plist
